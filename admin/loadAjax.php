				...
				if($uid == $_SESSION['id_login']){
					...
					<div style="float:right;" class="btn-group">
						  ...
						  <ul style="margin-left:-164px!important;margin-top:-25px!important;"class="dropdown-menu" role="menu">
							...
							<li><a class="stdelete" href="#" id="'.$msg_id.'" title="Delete update"><i class="fa fa-trash-o"></i> Excluir</a></li>
						  </ul>
						</div>
					';
				}else{
			...
			<div class="sttime">
				<?php
					...
					if(count($result)){
						...
						echo "<a class='unlike'><i class='fa fa-thumbs-o-down'></i> Descutir</a> <div class='likethis'><span class='badge'>".$total."</span></div> | <a href='#' class='commentopen' id='".$msg_id."' title='Comment'><i class='fa fa-comments-o'></i> Comentar </a>";
					}else{
						$result = $pdo->select("SELECT count(msg_id) as total FROM likes WHERE msg_id = ".$msg_id."");
						foreach($result as $res){
							$total = $res['total'];
						...
						if($total == 0){
							echo "<a class='like'><i class='fa fa-thumbs-o-up'></i> Curtir</a> <div class='likethis'></div> | <a href='#' class='commentopen' id='".$msg_id."' title='Comment'><i class='fa fa-comments-o'></i> Comentar </a>";
						}else{
							echo "<a class='like'><i class='fa fa-thumbs-o-up'></i> Curtir</a> <div class='likethis'><span class='badge'>".$total."</span></div> | <a href='#' class='commentopen' id='".$msg_id."' title='Comment'><i class='fa fa-comments-o'></i> Comentar </a>";
						}
									...
									echo '
											<div style="float:right;" class="btn-group">
												  ...
												  <ul style="margin-left:-164px!important;margin-top:-25px!important;"class="dropdown-menu" role="menu">
													...
													<li><a class="stcommentdelete" href="#" id="'.$com_id.'" title="Delete Comment"><i class="fa fa-trash-o"></i> Excluir</a></li>
												  </ul>
												</div>
											';
			...
			<div class="commentupdate" style='display:none' id='commentbox<?php echo $msg_id;?>'>
				...
				<div class="stcommenttext" >
					<form method="post" action="">
						...
						<br />
						<input type="submit"  value=" Comentar "  id="<?php echo $msg_id;?>" class="comment_button btn btn-primary btn-xs"/>
					</form>
				</div>
			</div>
		</div> 
